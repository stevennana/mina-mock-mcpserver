...
- approving consent creates a single-use-ready authorization code bound to client, redirect URI, user, selected endpoints, resource/audience, optional PKCE challenge, state, expiry, and unused status, then redirects with `code` and `state`
...
- Authorization-code browser flow selects endpoint permissions, exchanges code for token with and without PKCE where applicable, calls /mcp/oauth successfully for allowed endpoint, receives 403 for denied endpoint, revokes token and receives 401
