...
- OAuth user, client, token filter, and permission inputs expose concise hover tooltips explaining login credentials, token TTLs, client credentials, redirect URIs, allowed tools/resources/prompts, grants, subjects, and client filters
...
- OAuth clients store redirect URIs, client credentials TTL presets, and maximum allowed tool/resource/prompt sets that later consent and client credentials flows must not exceed
...
- consent displays client, redirect URI, resource, user, authorization-code TTL, and grouped Tools, Resources, and Prompts permission checklists constrained to the client's allowed sets
- approving consent creates a single-use-ready authorization code bound to client, redirect URI, user, selected tools/resources/prompts, resource/audience, optional PKCE challenge, state, expiry, and unused status, then redirects with `code` and `state`
...
- client-credentials tokens use the client credentials TTL, subject `client:<client_id>`, `grant_type=client_credentials`, and permissions equal to all client-allowed tools/resources/prompts when no scope is requested or the intersection of requested `endpoint:<endpoint_id>`, `resource:<resource_id>`, and `prompt:<prompt_id>` scopes and client-allowed sets when scope is present
- access tokens are RS256 JWTs with issuer, audience/resource, subject, client ID, grant type, issued/expiry times, `jti`, scope, `endpoint_permissions`, `resource_permissions`, and `prompt_permissions`
...
- token detail shows reconstructed JWT claims and tool/resource/prompt permission metadata from stored records, but does not redisplay raw access token values after issuance
...
- Non-built-in OAuth client CRUD validates client ID shape, redirect URI shape, TTL presets, unique client IDs, and existing allowed tool/resource/prompt IDs
...
- Consent permission selection must be a subset of the registered OAuth client's allowed tool/resource/prompt IDs
- Client-credentials requested scopes must never expand beyond the registered OAuth client's allowed tool/resource/prompt IDs
...
- Issued-token catalog remains list/filter focused; token claims, tool/resource/prompt permissions, and revoke actions are shown on token detail pages
- Authorization-code browser flow selects tool/resource/prompt permissions, exchanges code for token with and without PKCE where applicable, calls /mcp/oauth successfully for allowed items, receives 403 for denied tool/resource/prompt access, revokes token and receives 401
