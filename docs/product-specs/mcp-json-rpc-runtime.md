...
The reusable JSON-RPC method handling should live behind the `@minasoft/mcp-runtime` package boundary described in `../design-docs/mcp-runtime-library-architecture.md`. MCP Mock Server remains responsible for auth, CORS, SSE route hosting, endpoint matching, fixture persistence, and audit evidence, but it should not keep a second app-local implementation of reusable MCP protocol behavior.
...
- Package extraction validation must prove MCP Mock Server consumes `@minasoft/mcp-runtime` for reusable protocol handling instead of keeping active imports from app-local MCP protocol/type modules.
