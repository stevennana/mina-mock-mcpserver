...
import { createMockServerRuntimeProvider } from "@/lib/mcp/runtime-provider";
...
async function handleMcpJsonRpcPost(
  ...
  const result = await handleMcpJsonRpcMessage(
    body,
    createMockServerRuntimeProvider({
      ...runtime,
  ...
  const status = "error" in result.body && result.body.error.code === -32003 ? 403 : result.status;
  return NextResponse.json(result.body, { status, headers: mcpResponseHeaders() });
}
